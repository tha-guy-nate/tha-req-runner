...
- Superseded 0.2.9's fix below — re-locking for `httpx2` forces a fresh `anyio` resolution independent of the old `httpx` pin, so `anyio` is confirmed clean at `4.15.1` here too.
